...
- **Status:** Deployed to prod, cron scheduled Sun 17:00 CT (= 18:00 ET).
- **First end-to-end dry-run validated 2026-05-11:** 7 steps, 24 LLM calls, 46 tool calls, $0.50/run, 4.7 minutes wall clock. Produced a 4-story draft (ND wheat acreage lead, ND corn brief, High Plains wheat belt feature, Montana spring wheat explainer) that read like a real analyst's weekly. Fact-checker caught 2 over-interpretations + 2 derived numbers (working as designed).
- **Architecture:** signal_board (deterministic, 10 sources) → mood (Sonnet 4.6, JSON) → editor (Sonnet 4.6, JSON) → researcher (Sonnet 4.6 + 5 tools, 30-call cap) → writer (Sonnet 4.6) → fact-checker (deterministic tokenizer + Haiku 4.5 critique) → publisher (S3 + RDS + Slack + magic-link tokens).
- **Signal sources (10):** yield, acreage, price, wasde, weather (drought + CCI + precip), exports, trend-break, calendar — all anomaly-driven. Composites bundle same-week multi-commodity narratives (e.g. "Export commitments diverges: soy -49% / corn +20%"). Feature signals provide educational/structural counterweight: state-trend rotations, regional cluster spotlights (Ogallala, Corn Belt, Mississippi Delta, etc), seasonal explainers (WASDE 101, pollination primer, corn/soy ratio, drought monitor reading guide).
- **Endpoints live at `https://agri-intel.rvedire.com/api/v1/agent/*`**: `/runs`, `/promote/{id}`, `/reject/{id}`, `/draft/{slug}/auth`, `/markdown/{slug}`, `/chart/{slug}/{name}`. Promote + reject gated by `FIELDPULSE_DRAFT_SECRET` shared-secret header.
- **Frontend at `/insights`** (Vercel): public list view, `/insights/{slug}` reader, `/insights/draft/{slug}` gated reader with Approve/Reject buttons. Signed-cookie auth via one-shot magic-link tokens posted to Slack.
- **DB (alembic 012):** 5 new tables — `agent_runs`, `agent_picks`, `agent_mood`, `agent_settings` (singleton kill-switch, seeded `force_manual=true`), `agent_draft_tokens` (one-shot magic-link tokens).
- **Trust-period exit:** auto-publish flips on after 6 consecutive `approved` runs with no `failed_at_step` (configurable via `AGENT_TRUST_STREAK_REQUIRED`). Until then every Sunday's draft sits in `newsletters/draft/<slug>/` awaiting Slack-pinged approval.
- **SQL guard (§7.1):** every researcher tool takes a runtime-injected `as_of_date` the LLM cannot override. SQL tool rejects DML/DDL + unknown tables via sqlglot AST traversal; injects `<col> <= :as_of` predicates per allowlisted table. Verified rejecting `DELETE`/`UPDATE`/`DROP`/`SELECT FROM users`.
- **Calibration:** 77 signals labelled 2026-05-11 but 75/2 positive/negative skew was too imbalanced for a useful AUC fit. Weights moved to `data/weights.degenerate.json`; system runs on `DEFAULT_WEIGHTS` (0.5 magnitude + 0.25 reach + 0.15 novelty + 0.10 calendar). Worth relabelling with more discriminating "would this be a story?" judgements once a few real issues run.
- **Known gaps (non-blocking):**
  - `acreage_accuracy` history is 5 years deep — state-trend signals show 4-year windows instead of the 25-year arcs the prompt intends. Backfilling historical state NASS would enable proper 25-year stories.
...
#### Model newsletter: chart-enabled issue reference (2026-06-12)

Built `/insights/model`, a fully designed reference issue showing what the future chart-enabled publisher step should produce instead of `{{chart_N}}` placeholders. Content is the 2026-05-03 dry-run draft re-expressed as a typed **IssueSpec** (the JSON contract a new agent "issue composer" step would emit), rendered with live Recharts charts, KPI strips, stat callouts, and a county-textured region map.

- **Spec contract:** `web_app/src/components/insights/model/types.ts` — `IssueSpec = { blocks: Block[], meta }`. Block kinds: `title`, `dek`, `section` (lead flag), `brief`, `p`, `watch`, `kpis` (glance strip), `stat` (pull-stat callout), `figure` (1 chart = full width, 2 = side-by-side panel pair), `hr`. Chart types: `bars` (horizontal, signed-pct or abs formats, custom zero-side labels so negative bars don't collide with the category axis), `trend_forecast` (actuals line + dashed bridge + forecast dot with P10-P90 ErrorBar whisker + reference line), `region_map` (state choropleth built from `/us-counties.geojson` filtered + cropped to the spec's states, county texture, hatch pattern for flagged states, theme-aware diverging ramp).
- **Renderer:** `ModelIssue.tsx` maps spec → components. Prose blocks reuse the `.fp-issue-*` typography classes so spec-rendered issues read identically to markdown ones. Rich blocks use new `.fpn-*` classes in `globals.css` (banner, glance KPI grid, stat callout, figure card with Barlow title + mono source line, trend legend; responsive at 720px).
- **Files:** `web_app/src/components/insights/model/{types.ts, data.ts, charts.tsx, RegionMap.tsx, ModelIssue.tsx}` + `web_app/src/app/insights/model/page.tsx` (robots noindex, MODEL ISSUE banner).
- **Data honesty:** prose is verbatim from the draft; a few series values the draft only gives as ranges (ND 2021-24 yearly acres, 2026 P10, NE/CO/SD belt-map values) are placeholders flagged in each figure's source line.
- **Verified** in browser preview: light + dark mode (map ramp switches palettes via a `data-theme` MutationObserver), mobile 375px (KPIs 2-col, panel pairs stack), `tsc --noEmit` clean, no console errors.
- **Agent integration path:** the composer step emits IssueSpec JSON (charts chosen from the researcher's data pulls), publisher stores it alongside the markdown, and the `[slug]` reader renders `ModelIssue` when a spec exists, falling back to `IssueRenderer` otherwise.

#### Composer step: spec-rendered issues live (2026-06-12)

Implemented the integration path above. New pipeline step between fact-check and publish: **composer** (deterministic prose conversion + ONE Sonnet design call, ~$0.06/run measured).

- **Architecture (the load-bearing decision):** prose is NEVER touched by an LLM. `backend/agent/composer.py::parse_markdown_blocks` is a Python port of `IssueRenderer.tsx` parsing (headings, dek, watch-sentence split + heuristics — cross-reference comments in both files), so spec prose is byte-identical to the fact-checked markdown. The LLM ("visual designer", `prompts/composer_system.txt`) only emits rich blocks: one kpis strip, stat callouts, figures with chart data copied from researcher chart_specs. Placement via `after_block` indices + `{{chart_N}}` anchor positions.
- **Numeric guard:** every number in rich blocks (KPI values/captions, stat values, figure titles, raw chart series) re-verified against the dossier via the factcheck tokenizer (public aliases added in `factcheck.py`) + a raw-number pool with power-of-1000 scale tolerance + derivation check. Ungroundable blocks are dropped, not revised. First dry run: guard killed 1 invented figure, pydantic validation killed 1 malformed one — both correctly.
- **Degradation chain:** composer crash / designer failure / zero surviving figures (when draft has chart specs) → `spec=None` → today's markdown+PNG publish. `_step_compose` swallows its own exceptions (a composer bug must never fail a run). Pre-composer published issues 404 on the spec endpoint → frontend falls back to `IssueRenderer`.
- **Contract:** `backend/agent/issue_spec.py` (pydantic v2) mirrors `model/types.ts` exactly, TS camelCase preserved. Validated end-to-end before upload.
