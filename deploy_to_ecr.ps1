# deploy_to_ecr.ps1 - Build and push Docker image to Amazon ECR (using AWS SSO)
#
# Usage:
#   1. Edit the variables below (AWS_ACCOUNT_ID, AWS_REGION, REPO_NAME, SSO_PROFILE)
...
#   - Permissions for ECR operations via IAM role
...
# Get it with: aws sts get-caller-identity --profile YOUR_SSO_PROFILE
...
# AWS SSO Profile name (configured via: aws configure sso)
# Leave empty "" to use default profile
$SSO_PROFILE = "your-sso-profile-name"
...
Write-Host "Deploying to Amazon ECR (using AWS SSO)"
Write-Host $separator
...
if ($SSO_PROFILE -ne "") {
    Write-Host "SSO Profile: $SSO_PROFILE"
}
Write-Host $separator
Write-Host ""

# Build AWS CLI profile argument
$profileArg = if ($SSO_PROFILE -ne "") { "--profile $SSO_PROFILE" } else { "" }

# Step 0: Ensure AWS SSO session is active
Write-Host "Step 0: Checking AWS SSO login..."
try {
    $identity = Invoke-Expression "aws sts get-caller-identity $profileArg --output json" | ConvertFrom-Json
    Write-Host "Logged in as: $($identity.Arn)" -ForegroundColor Green
} catch {
    Write-Host "AWS SSO not logged in. Running: aws sso login $profileArg" -ForegroundColor Yellow
    Invoke-Expression "aws sso login $profileArg"
...
try {
    $loginCommand = Invoke-Expression "aws ecr get-login-password --region $AWS_REGION $profileArg"
...
Write-Host "  aws apprunner start-deployment --service-arn YourServiceArn $profileArg"
Write-Host $separator